...
-- Backfill slugs for existing study_materials (with unique handling)
UPDATE study_materials SET slug = lower(regexp_replace(title, '[^a-zA-Z0-9]+', '-', 'g')) || '-' || id WHERE slug IS NULL OR slug = '';

-- Backfill slugs for existing blog_posts (with unique handling)
UPDATE blog_posts SET slug = lower(regexp_replace(title, '[^a-zA-Z0-9]+', '-', 'g')) || '-' || id WHERE slug IS NULL OR slug = '';
