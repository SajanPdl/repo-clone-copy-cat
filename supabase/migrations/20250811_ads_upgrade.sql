-- Ads system upgrade: campaigns, creatives, metrics, RPCs, policies
...
create table if not exists public.ad_campaigns (
  ...
  name text not null,
  placement text not null, -- header, footer, sidebar, inline, popup, floater
  start_at timestamptz not null default now(),
  end_at timestamptz,
  priority int not null default 1, -- higher wins
  daily_cap int,
  hourly_cap int,
...
create table if not exists public.ad_creatives (
  ...
  title text,
  description text,
  media_url text not null, -- image/video
...
create policy admin_all_advertisers on public.advertisers for all using (public.is_admin(auth.uid()));

drop policy if exists admin_all_campaigns on public.ad_campaigns;
create policy admin_all_campaigns on public.ad_campaigns for all using (public.is_admin(auth.uid()));

drop policy if exists admin_all_creatives on public.ad_creatives;
create policy admin_all_creatives on public.ad_creatives for all using (public.is_admin(auth.uid()));

-- Public read creatives (serving)
drop policy if exists public_read_creatives on public.ad_creatives;
create policy public_read_creatives on public.ad_creatives for select using (true);

-- Metrics insertion allowed for anyone (no reads)
drop policy if exists insert_impressions on public.ad_impressions;
create policy insert_impressions on public.ad_impressions for insert with check (true);

drop policy if exists insert_clicks on public.ad_clicks;
create policy insert_clicks on public.ad_clicks for insert with check (true);

-- 3) RPC: get_active_ads
...
-- 4) RPC: log metrics
create or replace function public.log_ad_impression(p_creative uuid, p_campaign uuid, p_user uuid default null, p_country text default null, p_device text default null)
...
create or replace function public.log_ad_click(p_creative uuid, p_campaign uuid, p_user uuid default null, p_country text default null, p_device text default null)
returns void language plpgsql security definer as $$
begin
