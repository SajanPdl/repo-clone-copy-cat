...
-- Run this in your Supabase SQL Editor to diagnose what's missing
...
-- ===========================================
-- STEP 6: Check current user and admin status
-- ===========================================
...
SELECT
    ...
    CASE
        WHEN EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND is_admin = true)
        THEN '✅ IS ADMIN'
        ELSE '❌ NOT ADMIN'
    END as admin_status;
...
SELECT
    ...
    qual
FROM pg_policies
WHERE tablename IN ('subscription_plans', 'user_subscriptions', 'payment_requests')
